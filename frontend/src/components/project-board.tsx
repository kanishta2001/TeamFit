...
export default function ProjectBoard({ user, projects, filter }: {
  ...
  return <section className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-2xl font-bold">{title}</h1><p className="mt-2 text-slate-600">
      ...
      <Link className={buttonStyle} href="/projects/new">Create project</Link>
    </div>
    ...
    <label className="block text-sm font-semibold">Search projects<input className={inputStyle} value={search} onChange={event => setSearch(event.target.value)} /></label>
